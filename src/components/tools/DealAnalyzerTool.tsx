...
export function DealAnalyzerTool({ locale }: Props) {
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      pdf.save('deal-analyzer-underwriting-summary.pdf');
    } catch (err) {
  ...
  return (
    <div className="space-y-8 mb-12">
      ...
      {/* Hidden Printable PDF Card (Used by html2canvas for PDF Generation) */}
      <div className="absolute left-[-9999px] top-[-9999px]">
        ...
        >
          ...
          <div className="border-b border-slate-300 pb-4 flex justify-between items-center">
            <div>
              <div className="flex items-center gap-2 text-emerald-700 font-bold text-xl">
                ...
                <span>CRE Calculators</span>
              </div>
              <h1 className="text-2xl font-black text-slate-900 mt-1">
                {isZh ? '商业地产尽调与投资分析摘要 (Deal Analysis Summary)' : 'Commercial Real Estate Underwriting Summary'}
              </h1>
            </div>
          ...
          <div className={`p-4 rounded-xl border ${baseBadge.bg} flex items-center justify-between`}>
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500 block">{isZh ? '交易健康度判定' : 'Deal Health Evaluation'}</span>
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2">
            <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">{isZh ? '关键输入参数' : 'Underwriting Input Parameters'}</h3>
            <div className="grid grid-cols-3 gap-3 text-xs text-slate-700">
              ...
              <div><strong>{isZh ? '总潜在收入 (GPI):' : 'GPI:'}</strong> {formatCurrency(grossPotentialIncome)}</div>
              <div><strong>{isZh ? '空置率:' : 'Vacancy Rate:'}</strong> {vacancyRate}%</div>
              <div><strong>{isZh ? '有效毛收入 (EGI):' : 'EGI:'}</strong> {formatCurrency(base.egi)}</div>
              ...
              <div><strong>{isZh ? '年利率 / 摊销:' : 'Rate / Amort:'}</strong> {interestRate}% / {amortizationYears} Yrs</div>
            </div>
          </div>

          {/* Core Results Table */}
          <div className="space-y-2">
            <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">{isZh ? '核心评估指标计算结果' : 'Calculated Core CRE Metrics'}</h3>
            <table className="w-full text-xs text-left border-collapse border border-slate-300">
              <thead className="bg-slate-900 text-white font-bold">
                <tr>
                  <th className="p-2.5 border border-slate-700">{isZh ? '评估指标' : 'Metric'}</th>
                  <th className="p-2.5 border border-slate-700">{isZh ? '基础情景 (Base)' : 'Base Scenario'}</th>
                  <th className="p-2.5 border border-slate-700">{isZh ? '压力情景 (+5% Vac, +1% Rate)' : 'Stress Scenario (+5% Vac, +1% Rate)'}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200">
                ...
                <tr>
                  <td className="p-2.5 font-bold border border-slate-200">{isZh ? '现金回报率 (Cash-on-Cash)' : 'Cash-on-Cash Return'}</td>
          ...
          <div className="pt-4 border-t border-slate-300 text-center text-[10px] text-slate-500">
            {isZh
              ? '本文档由 CRE Calculators (https://cre-calculator.pages.dev) 自动生成，仅供投资尽调初筛使用。'
              : 'Generated by CRE Calculators (https://cre-calculator.pages.dev). For underwriting screening purposes only.'}
          </div>
        </div>
      </div>
    </div>
